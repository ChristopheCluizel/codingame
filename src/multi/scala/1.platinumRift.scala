...
class Graph(val nbPlayers: Int, val myId: Int, val nbTotalZones: Int, val nbTotalLinks: Int) {
    ...
    def isHalfPlatinumZonesOwned: Boolean = {
        ...
        return (nbPlatinumZonesOwned >= (platinumZones.length / 4))
    }
    ...
    private def spawnNextToFreeZone(idZone: Int): String = {
        ...
        if(!nodes(idZone).isZoneWithEnemy(myId)){
    ...
    def main(args: Array[String]) {
        ...
        while(true) {
            ...
            if(firstTurn){
                ...
                var counter = 0
                println(graph.move)
                var platinumZoneOrdered = graph.platinumZones.sortBy(x => graph.nodes(x).platinumSource)
                platinumZoneOrdered = platinumZoneOrdered.reverse
                while(graph.myPlatinum >= 20){
                    idZone = platinumZoneOrdered(counter)
                    graph.myPlatinum -= 20
                    counter += 1
                    orders += "1 " + idZone + " "
                }
